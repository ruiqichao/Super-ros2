...
namespace traj_opt {
    ...
    /**
     * @brief SuperPlanner轨迹优化器配置类，支持动态参数调整
     * @note 此类封装了轨迹优化器的可动态调整参数
     */
    ...
    public:
        ...
        // 算法配置参数
        int pos_constraint_type{2};       // 位置约束类型: 1-路径点约束, 2-走廊约束
        bool block_energy_cost{false};    // 是否禁用能量代价
                ...
                if (type == rcl_interfaces::msg::ParameterType::PARAMETER_DOUBLE) {
                    ...
                    if (name == "traj_opt.boundary.max_vel") {
                        ...
                        RCLCPP_INFO(rclcpp::get_logger("super_planner"), 
                                   "Parameter updated: max_vel = %f", max_vel);
                    } else if (name == "traj_opt.boundary.max_acc") {
                        max_acc = parameter.as_double();
                        RCLCPP_INFO(rclcpp::get_logger("super_planner"), 
                                   "Parameter updated: max_acc = %f", max_acc);
                    } else if (name == "traj_opt.boundary.max_jerk") {
                        max_jerk = parameter.as_double();
                        RCLCPP_INFO(rclcpp::get_logger("super_planner"), 
                                   "Parameter updated: max_jerk = %f", max_jerk);
                    }
                    // 惩罚系数参数
                    else if (name == "traj_opt.exp_traj.penna_t") {
                        penna_t = parameter.as_double();
                        RCLCPP_INFO(rclcpp::get_logger("super_planner"), 
                                   "Parameter updated: penna_t = %f", penna_t);
                    } else if (name == "traj_opt.exp_traj.penna_pos") {
                        penna_pos = parameter.as_double();
                        RCLCPP_INFO(rclcpp::get_logger("super_planner"), 
                                   "Parameter updated: penna_pos = %f", penna_pos);
                    } else if (name == "traj_opt.exp_traj.penna_vel") {
                        penna_vel = parameter.as_double();
                        RCLCPP_INFO(rclcpp::get_logger("super_planner"), 
                                   "Parameter updated: penna_vel = %f", penna_vel);
                    } else if (name == "traj_opt.exp_traj.penna_acc") {
                        penna_acc = parameter.as_double();
                        RCLCPP_INFO(rclcpp::get_logger("super_planner"), 
                                   "Parameter updated: penna_acc = %f", penna_acc);
                    } else if (name == "traj_opt.exp_traj.penna_jerk") {
                        penna_jerk = parameter.as_double();
                        RCLCPP_INFO(rclcpp::get_logger("super_planner"), 
                                   "Parameter updated: penna_jerk = %f", penna_jerk);
                    } else if (name == "traj_opt.exp_traj.penna_attract") {
                        penna_attract = parameter.as_double();
                        RCLCPP_INFO(rclcpp::get_logger("super_planner"), 
                                   "Parameter updated: penna_attract = %f", penna_attract);
                    } else if (name == "traj_opt.exp_traj.penna_omg") {
                        penna_omg = parameter.as_double();
                        RCLCPP_INFO(rclcpp::get_logger("super_planner"), 
                                   "Parameter updated: penna_omg = %f", penna_omg);
                    } else if (name == "traj_opt.exp_traj.penna_thr") {
                        penna_thr = parameter.as_double();
                        RCLCPP_INFO(rclcpp::get_logger("super_planner"), 
                                   "Parameter updated: penna_thr = %f", penna_thr);
                    }
                    // 优化参数
                    else if (name == "traj_opt.exp_traj.opt_accuracy") {
                        opt_accuracy = parameter.as_double();
                        RCLCPP_INFO(rclcpp::get_logger("super_planner"), 
                                   "Parameter updated: opt_accuracy = %f", opt_accuracy);
                    } else if (name == "traj_opt.exp_traj.smooth_eps") {
                        smooth_eps = parameter.as_double();
                        RCLCPP_INFO(rclcpp::get_logger("super_planner"), 
                                   "Parameter updated: smooth_eps = %f", smooth_eps);
                    }
                }
                else if (type == rcl_interfaces::msg::ParameterType::PARAMETER_INTEGER) {
                    // 积分分辨率参数
                    if (name == "traj_opt.exp_traj.integral_reso") {
                        integral_reso = parameter.as_int();
                        RCLCPP_INFO(rclcpp::get_logger("super_planner"), 
                                   "Parameter updated: integral_reso = %d", integral_reso);
                    }
                    // 算法配置参数
                    else if (name == "traj_opt.exp_traj.pos_constraint_type") {
                        pos_constraint_type = parameter.as_int();
                        RCLCPP_INFO(rclcpp::get_logger("super_planner"), 
                                   "Parameter updated: pos_constraint_type = %d", pos_constraint_type);
                    }
                }
                else if (type == rcl_interfaces::msg::ParameterType::PARAMETER_BOOL) {
                    // 算法配置参数
                    if (name == "traj_opt.exp_traj.block_energy_cost") {
                        block_energy_cost = parameter.as_bool();
                        RCLCPP_INFO(rclcpp::get_logger("super_planner"), 
                                   "Parameter updated: block_energy_cost = %s", block_energy_cost ? "true" : "false");
                    }
                }
            }
