...
BackupTrajOpt::BackupTrajOpt(const traj_opt::Config &cfg, const ros_interface::RosInterface::Ptr &ros_ptr)
    ...
    // 首先从cfg_中初始化dyn_cfg_的参数值
    {
        std::lock_guard<std::mutex> l(dyn_cfg_.getConfigMutex());
        dyn_cfg_.max_vel = cfg_.max_vel;
        dyn_cfg_.max_acc = cfg_.max_acc;
        dyn_cfg_.max_jerk = cfg_.max_jerk;
        dyn_cfg_.penna_t = cfg_.penna_t;
        dyn_cfg_.penna_pos = cfg_.penna_pos;
        dyn_cfg_.penna_vel = cfg_.penna_vel;
        dyn_cfg_.penna_acc = cfg_.penna_acc;
        dyn_cfg_.penna_jerk = cfg_.penna_jerk;
        dyn_cfg_.penna_attract = cfg_.penna_attract;
        dyn_cfg_.penna_omg = cfg_.penna_omg;
        dyn_cfg_.penna_thr = cfg_.penna_thr;
        dyn_cfg_.opt_accuracy = cfg_.opt_accuracy;
        dyn_cfg_.smooth_eps = cfg_.smooth_eps;
        dyn_cfg_.integral_reso = cfg_.integral_reso;
    }
    // 然后使用动态配置更新优化变量
    {
        std::lock_guard<std::mutex> l(dyn_cfg_.getConfigMutex());
        opt_vars.magnitudeBounds << dyn_cfg_.max_vel, dyn_cfg_.max_acc, dyn_cfg_.max_jerk,
                cfg_.max_omg, cfg_.min_acc_thr * cfg_.mass, cfg_.max_acc_thr * cfg_.mass;
        opt_vars.penaltyWeights << dyn_cfg_.penna_pos, dyn_cfg_.penna_vel,
                dyn_cfg_.penna_acc, dyn_cfg_.penna_jerk,
                dyn_cfg_.penna_attract, dyn_cfg_.penna_omg,
                dyn_cfg_.penna_thr;
        opt_vars.rho = dyn_cfg_.penna_t;
        opt_vars.pos_constraint_type = cfg_.pos_constraint_type;
        opt_vars.block_energy_cost = cfg_.block_energy_cost;
        opt_vars.smooth_eps = dyn_cfg_.smooth_eps;
        opt_vars.integral_res = dyn_cfg_.integral_reso;
...
void BackupTrajOpt::updateOptimizerConfig() {
    // 使用动态参数配置更新优化器内部参数
    std::lock_guard<std::mutex> l(dyn_cfg_.getConfigMutex()); // 确保线程安全
    ...
    opt_vars.smooth_eps = dyn_cfg_.smooth_eps;
    opt_vars.integral_res = dyn_cfg_.integral_reso;
    opt_vars.weight_ts = dyn_cfg_.penna_ts;  // 更新时间段惩罚系数
