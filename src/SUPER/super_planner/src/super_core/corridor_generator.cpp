...
//        failed_traj_log.open(DEBUG_FILE_DIR("sfc.csv"), std::ios::out | std::ios::trunc);
    ...
    /**
     * @brief 在路径上搜索多面体，生成安全走廊
     * @param path 路径点序列
     * @param sfcs 输出的安全走廊多面体序列
     * @param shifted_start_pt 偏移后的起始点
     * @param cut_first_poly 是否裁剪第一个多面体
     * @return 是否成功生成走廊
     */
    bool
...
//                        exit(-1);
     ...
     * @param p1 第一个点
     * @param p2 第二个点
     * @param box_min 输出的边界框最小值
     * @param box_max 输出的边界框最大值
     */
