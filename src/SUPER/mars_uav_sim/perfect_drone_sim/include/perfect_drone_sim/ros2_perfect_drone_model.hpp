...
namespace perfect_drone {
    ...
    public:
        PerfectDrone(): Node("perfect_tracking") {
            // Use reliable QoS for better compatibility with RViz
...
#define CONFIG_FILE_DIR(name) (std::string(std::string(ROOT_DIR) + "config/"+(name)))
        ...
        void publishPC() {
            ...
            // Only publish if there are points in the cloud
            if (local_map->empty()) {
                return;
            }
            
            // Downsample the point cloud to reduce data size
            pcl::PointCloud<marsim::PointType>::Ptr downsampled_map(new pcl::PointCloud<marsim::PointType>);
            pcl::VoxelGrid<marsim::PointType> voxel_filter;
            voxel_filter.setInputCloud(local_map);
            voxel_filter.setLeafSize(0.2f, 0.2f, 0.2f); // Adjust this value to control downsampling
            voxel_filter.filter(*downsampled_map);
            
            sensor_msgs::msg::PointCloud2 pc_msg;
            // Use PCL conversion that properly handles intensity field
            pcl::toROSMsg(*downsampled_map, pc_msg);
            pc_msg.header.frame_id = "world";
            pc_msg.header.stamp = this->get_clock()->now();
            
            std::cout << "Publish local map size: " << downsampled_map->size() << std::endl;
