...
// fmt formatter for GridType
template <>
struct fmt::formatter<super_utils::GridType> {
    template <typename ParseContext>
    constexpr auto parse(ParseContext &ctx) { return ctx.begin(); }

    template <typename FormatContext>
    auto format(const super_utils::GridType& gt, FormatContext &ctx) const {
        return fmt::format_to(ctx.out(), "{}", static_cast<int>(gt));
    }
};
