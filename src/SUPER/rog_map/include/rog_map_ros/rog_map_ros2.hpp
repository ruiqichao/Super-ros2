...
namespace rog_map {
    ...
    class ROGMapROS : public ROGMap {
        ...
        struct ROSCallback {
            ...
            rclcpp::Service<rog_map::srv::ResetMap>::SharedPtr reset_map_srv;
        ...
        void resetMapServiceCallback(const std::shared_ptr<rog_map::srv::ResetMap::Request> req,
                                      std::shared_ptr<rog_map::srv::ResetMap::Response> res) {
            std::cout << BLUE << " -- [ROGMapROS] Reset map service called" << RESET << std::endl;
            try {
                resetLocalMap();
                res->success = true;
                res->message = "Map reset successfully";
                std::cout << GREEN << " -- [ROGMapROS] Map reset completed" << RESET << std::endl;
            } catch (const std::exception &e) {
                res->success = false;
                res->message = std::string("Error: ") + e.what();
                std::cout << RED << " -- [ROGMapROS] Map reset failed: " << e.what() << RESET << std::endl;
            }
        }
    ...
    public:
        ...
        ROGMapROS(const rclcpp::Node::SharedPtr nh, const std::string& cfg_path): nh_(nh) {
            ...
            if (cfg_.ros_callback_en) {
                ...
                rc_.reset_map_srv = nh_->create_service<rog_map::srv::ResetMap>(
                    "rog_map/reset_map",
                    std::bind(&ROGMapROS::resetMapServiceCallback, this, std::placeholders::_1, std::placeholders::_2)
                );
                std::cout << GREEN << " -- [ROGMapROS] Reset map service advertised on /rog_map/reset_map" << RESET << std::endl;
            }
        }
